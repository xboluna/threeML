...
def download_LAT_data(
    ...
    try:
        urllib.urlretrieve(url, temporaryFileName, lambda x, y, z: 0, postData)
    ...
    try:
        ...
        estimatedTimeLine = filter(
            lambda x: x.find("The estimated time for your query to complete is") == 0,
            parser.data,
        )[0]
    ...
    http_address = filter(
        lambda x: x.find("https://fermi.gsfc.nasa.gov") >= 0, parser.data
    )[0]
