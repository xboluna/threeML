...
class Minimizer(object):
            ...
            for i in range(self.Npar):
                ...
                for j in range(self.Npar):
                    ...
                    if variance_i * variance_j > 0:
                        ...
                        self._correlation_matrix[i, j] = self._covariance_matrix[
                            i, j
    ...
    def restore_best_fit(self):
        ...
        for parameter_name, best_fit_value in zip(
            self.parameters.keys(), best_fit_values
        ):
    ...
    def _compute_covariance_matrix(self, best_fit_values):
        ...
        minima = map(
            lambda parameter: parameter._get_internal_min_value(),
            self.parameters.values(),
        )
        maxima = map(
            lambda parameter: parameter._get_internal_max_value(),
            self.parameters.values(),
        )
    ...
    def _get_one_error(self, parameter_name, target_delta_log_like, sign=-1):
        ...
        while repeats < 10:
            ...
            current_value, current_delta, current_min, current_max = self._internal_parameters[
                parameter_name
            ]
