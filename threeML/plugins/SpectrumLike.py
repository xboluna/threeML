...
class SpectrumLike(PluginPrototype):
    def __init__(
        ...
        self._background_spectrum, self._background_plugin, self._background_counts, self._scaled_background_counts = (
        ...
        self._observed_count_errors, self._back_count_errors = (
        ...
        if self._background_plugin is not None:
            ...
            for (
                par_name,
                parameter,
            ) in self._background_plugin.likelihood_model.parameters.items():
    ...
    def source_rate_error(self):
        ...
        if (self._background_noise_model is not None) or (
            ...
            src_rate_err = np.sqrt(
                (self.observed_count_errors / self._observed_spectrum.exposure) ** 2
                + (
    ...
    def view_count_spectrum(
        ...
        if True:  # non_used_mask.sum() > 0:
            ...
            top = (
                max(
                    [
                        max(background_rate_unrebinned / energy_width_unrebinned),
                        max(observed_rate_unrebinned / energy_width_unrebinned),
                    ]
                )
                * 1.5
            )
            ...
            bottom = (
                min(
                    [
                        min(background_rate_unrebinned / energy_width_unrebinned),
                        min(observed_rate_unrebinned / energy_width_unrebinned),
                    ]
                )
                * 0.8
            )
