...
class HAWCLike(PluginPrototype):
    ...
    def set_model(self, likelihood_model_instance):
        ...
        for id in range(self._model.get_number_of_extended_sources()):
            ...
            lon_min, lon_max, lat_min, lat_max = self._model.get_extended_source_boundaries(
                id
            )
    ...
    def plot_radial_profile(
        ...
        radii, excess_model, excess_data, excess_error, list_of_bin_names = self.get_radial_profile(
            ra,
            dec,
            bin_list,
            max_radius,
        ...
        if len(list_of_bin_names) == 1:
            ...
            width = 84
            title = "\n".join(
                tmptitle[i : i + width] for i in xrange(0, len(tmptitle), width)
            )
